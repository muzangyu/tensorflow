...
llvm::SmallVector<Operation*, 8> getMoveOplist(
  ...
  // ops in the middle IfRegions can be easily gained.
...
void MergeIfPerSegment(llvm::SmallVector<TF::IfRegionOp, 8>& if_op_segment,
  ...
  // when the merged IfRegions are in the moved op list.
  // We need remove those old IfRegions and add the new IfRegion into the list
